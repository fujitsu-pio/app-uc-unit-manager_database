...
import com.fujitsu.dc.gui.portal.service.CreateEnvironmentService;
        ...
        try {
            ...
            if (name != null) {
                ...
                if (userinfo.size() > 0) {
                    ...
                    if (lockValue >= PCSConstants.MAX_LOCK_STATUS) {
                    ...
                    } else if (lockValue < PCSConstants.MAX_LOCK_STATUS && verifiedAt == null) {
                        session = request.getSession(true);
                        ...
                        session.setAttribute("errorMessage",
                                PCSConstants.EMAIL_NOT_VERIFIED_MSG);
            ...
            isUserExist = loginService.validateUsernamePassword(name, password,
                    updatedAt);
                if (isUserExist && lockValue < PCSConstants.MAX_LOCK_STATUS
                        && verifiedAt != null) {
                ...
                if (organizationStatus == 0) {
                    ...
                    session.setAttribute("errorMessage",
                            PCSConstants.LOGIN_ORG_NOTACTIVATED_MSG);
                    response.sendRedirect(contextPath);
                } else {
                ...
                session.setAttribute("errorMessage",
                        PCSConstants.LOGIN_INVALID_CREDENTIALS_MSG);
                response.sendRedirect(contextPath);
            }
            } else {
                String contextPath = request.getContextPath();

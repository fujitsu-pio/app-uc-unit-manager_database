...
import io.personium.gui.portal.service.AdministratorManagementService;
...
public class MailNotifications {
    ...
    static {
        emailSmtpServer = PersoniumEnvironment.getInstance().getResourceBundle()
                .getString("EMAIL_SMTP_SERVER");
        smtpPort = PersoniumEnvironment.getInstance().getResourceBundle()
                .getString("SMTP_PORT");
        emailSender = PersoniumEnvironment.getInstance().getResourceBundle()
                .getString("EMAILSENDER");
        languageType = PersoniumEnvironment.getInstance().getResourceBundle().getString("LANGUAGE_TYPE");
    }
    ...
    protected boolean sendMailForEmailVerification(String username,
        ...
        int constMiliSeconds = PersoniumConstants.CONSTMILLISECONDS;
        try {
            File file = getMailTemplateFilePath("VERIFICATION_MAIL");
            if (file != null) {
                String text = getMailTemplateFileContents(file);
                String verificationMailSubject = PersoniumEnvironment.getInstance()
                ...
                String strVerificationMailExpirationTime = PersoniumEnvironment
        ...
        try {
            ...
            if (file != null) {
                ...
                if (subscriberInfo.size() > 0 && userInfo.size() > 0) {
                    ...
                    String mailBody = mailTemplateParser
                            .updateMailTemplateWithDynamicData(mapText, text);
                    String subscriberMailSubject = PersoniumEnvironment.getInstance()
        ...
        try {
            ...
            if (userInfo.size() > 0) {
                ...
                if (file != null) {
                    ...
                    String mailBody = mailTemplateParser
                            .updateMailTemplateWithDynamicData(mapText, text);
                    String loginCredentialMailSubject = PersoniumEnvironment.getInstance()
                            .getResourceBundle()
        ...
        try {
            ...
            if (userInfo.size() > 0) {
                ...
                if (file != null) {
                    ...
                    String mailBody = mailTemplateParser
                            .updateMailTemplateWithDynamicData(mapText, text);
                    String forgetPasswordMailSubject = PersoniumEnvironment.getInstance()
                            .getResourceBundle()
    ...
    protected File getMailTemplateFilePath(String mailType) {
        ...
        if (languageType.equals("JA")) {
            defaultMailTemplatePath = "mail_templates/JA";
        }
        if (mailType == "VERIFICATION_MAIL") {
            fileName = PersoniumConstants.VERIFICATION_MAIL_TEMPLATE_FILE_NAME;
        } else if (mailType == "ACCOUNT_ACTIVATE_MAIL") {
            fileName = PersoniumConstants.ACTIVATE_ACCOUNT_MAIL_TEMPLATE_FILE_NAME;
        } else if (mailType == "LOGIN_CREDENTIAL_MAIL") {
            fileName = PersoniumConstants.LOGIN_CREDENTIAL_MAIL_TEMPLATE_FILE_NAME;
        } else if (mailType == "ACCOUNT_LOCKED_MAIL") {
            fileName = PersoniumConstants.ACCOUNT_LOCKED_MAIL_TEMPLATE_FILE_NAME;
        } else if (mailType == "USER_DELETED_MAIL") {
            fileName = PersoniumConstants.USERS_DELETED_LOCKED_MAIL_TEMPLATE_FILE_NAME;
        } else if (mailType == "FORGET_PASSWORD_MAIL") {
            fileName = PersoniumConstants.USERS_FORGET_PASSWORD_MAIL_TEMPLATE_FILE_NAME;
        } else if (mailType == "PASSWORD_CHANGE_MAIL") {
            fileName = PersoniumConstants.CHANGE_PASSWORD_MAIL_TEMPLATE_FILE_NAME;
        } else if (mailType == "PASSWORD_CHANGE_MAIL_BY_SUBSCRIBER") {
            fileName = PersoniumConstants.CHANGE_PASSWORD_BY_SUBSCRIBER_MAIL_TEMPLATE_FILE_NAME;
        }
    ...
    protected String getMailTemplateFileContents(File file) throws IOException {
        String fileContents = new String(Files.readAllBytes(Paths.get(file
                .getPath())), PersoniumConstants.UTF8);
        return fileContents;
    }
    /**
    ...
    protected boolean sendMailForAccountLocked(String username, String emailTo,
        ...
        try {
            ...
            if (file != null) {
                String text = getMailTemplateFileContents(file);
                String accountLockedMailSubject = PersoniumEnvironment.getInstance()
    ...
    protected boolean sendMailToSubscriberForUsersDeleted(String username, String[] deletedUserList) {
        ...
        try {
            ...
            if (file != null) {
                String text = getMailTemplateFileContents(file);
                String userDeletedMailSubject = PersoniumEnvironment.getInstance()
    ...
    protected String forgotPassword(HttpServletRequest request, String operation) {
        ...
        } catch (PersoniumCustomException pcsExe) {
    ...
    protected String getVerificattionMailStatu(HttpServletRequest request,
        ...
        try {
            String emailTo = URLDecoder.decode(
                    request.getParameter("encodedEAddress"), PersoniumConstants.UTF8);
            String firstName = URLDecoder.decode(
                    request.getParameter("firstName"), PersoniumConstants.UTF8);
            String familyName = URLDecoder.decode(
                    request.getParameter("familyName"), PersoniumConstants.UTF8);
        ...
        try {
            ...
            if (userInfo.size() > 0) {
                ...
                if (file != null) {
                    ...
                    String mailBody = mailTemplateParser
                            .updateMailTemplateWithDynamicData(mapText, text);
                    String changePasswordMailSubject = PersoniumEnvironment.getInstance()
                            .getResourceBundle()

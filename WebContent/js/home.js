 ...
 * @param unitURL
 */
home.prototype.getEnvDetail = function(envName, unitURL, envID) {
	...
	//if (isEnvtWindowOpened == false) {
		...
		uHome.setEnvironmentVariablesInLocalStorage(envID, envName, unitURL);
...
home.prototype.storeEnvDetails = function() {
	uHome.checkUserPrivilege();
};

/**
 * The purpose of this function is to check user privilege.
 */
home.prototype.checkUserPrivilege = function() {
	$.ajax({
		dataType : 'json',
		url : '../../home',
		type : 'GET',
		...
		success : function(jsonData) {
			if (jsonData.privilege == "sessionTimeOut") {
				...
				uHome.getUserPrivilege(jsonData);
			}
		},
		error : function(jsonData) {
			uCreateEnvironment.performLogout();
 ...
 * @param jsonData
 */
home.prototype.getUserPrivilege = function(jsonData) {
...
home.prototype.setEnvironmentVariables = function() {
	if (localStorage.clickedEnvironmentName != null
			&& localStorage.clickedEnvironmentUnitUrl != null
			&& localStorage.clickedEnvironmentId != null) {
		sessionStorage.selectedEnvName = localStorage
				.getItem("clickedEnvironmentName");
...
home.prototype.removeEnvironmentVariablesFromLocalStorage = function() {
	localStorage.removeItem("clickedEnvironmentName");
	localStorage.removeItem("clickedEnvironmentUnitUrl");
	localStorage.removeItem("clickedEnvironmentId");
};

home.prototype.setEnvironmentVariablesInLocalStorage = function (envID, envName, unitURL) {
	localStorage.setItem("clickedEnvironmentId", envID);
	localStorage.setItem("clickedEnvironmentName", envName);
